...
static ASTNode *parse_if_statement(Parser *parser)
{
    ...
    int elseif_capacity = 4;
    int elseif_count = 0;
    ASTNode **elseif_conditions = memory_allocate(sizeof(ASTNode *) * elseif_capacity);
    ASTNode **elseif_blocks = memory_allocate(sizeof(ASTNode *) * elseif_capacity);

    while (parser->current_token->type == TOKEN_ELSEIF)
    {
        parser_advance(parser);
        parser_expect(parser, TOKEN_LPAREN);
        ASTNode *elseif_condition = parse_expression(parser);
        parser_expect(parser, TOKEN_RPAREN);
        ...
        ASTNode *elseif_block = parse_block(parser);


        if (elseif_count >= elseif_capacity)
        {
            elseif_capacity *= 2;
            elseif_conditions = memory_reallocate(elseif_conditions, sizeof(ASTNode *) * elseif_capacity);
            elseif_blocks = memory_reallocate(elseif_blocks, sizeof(ASTNode *) * elseif_capacity);
        }

        elseif_conditions[elseif_count] = elseif_condition;
        elseif_blocks[elseif_count] = elseif_block;
        elseif_count++;
    }
    ...
    return ast_create_if_with_elseif(condition, then_block, elseif_conditions, elseif_blocks, elseif_count, else_block);
}
