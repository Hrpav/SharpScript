...
static Value *eval_node(Interpreter *interp, ASTNode *node)
{
    ...
    case AST_IF:
    {
        ...
        for (int i = 0; i < node->data.if_stmt.elseif_count; i++)
        {
            Value *elseif_condition = eval_node(interp, node->data.if_stmt.elseif_conditions[i]);
            int elseif_is_true = value_is_truthy(elseif_condition);
            value_free(elseif_condition);

            if (elseif_is_true)
            {
                return eval_node(interp, node->data.if_stmt.elseif_blocks[i]);
            }
        }
